...
- Service/route type aliases live in `packages/types/src/makaseLawTypes.ts`, not inlined: `Foo = Selectable<DB["foo"]>` for reads, `NewFoo = Insertable<DB["foo"]>` for create, `Partial<Foo>` for update. When adding a new table, add both `Foo` and `NewFoo` aliases here and import from `@makase-law/types` — never inline `Insertable<DB["foo"]>`/`Updateable<DB["foo"]>` in services
- `packages/types/src/remappedDB.ts` exposes `DB` as `RemapViews<_DB>` so `DB["foo"]` (view) resolves to the underlying `_foo` table type, preserving NOT NULL + `Generated<...>` that Postgres drops at the view layer; codegen column overrides (e.g. `role_config: OfficeRoleConfig`) must be applied to both `foo.col` and `_foo.col` to survive the remap
