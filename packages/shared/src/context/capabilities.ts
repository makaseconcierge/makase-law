import { getEmployeeContext } from "./loggedInContext";

export type Scope = "office" | "team" | "self";

/**
 * Resolves the caller's scope for `(capability, action)` from the merged
 ...
 */
export function getCapabilityScope(capability: string, action: string): Scope {
  const { isAdmin, isSystem, permissions } = getEmployeeContext();
  if (isAdmin || isSystem) return "office";
  const scope = permissions[capability]?.[action];
  if (!scope) {
